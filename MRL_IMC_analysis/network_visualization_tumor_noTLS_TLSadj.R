...
# write.csv(totalcounts_celltype_region, paste0(workd,"/Results/Totalcounts_T_region.csv"))
...
# Exclude regions that are adjacent to tumoral regions since you want to look at intratumoral only
ggdft <- ggdft[ggdft$Region!="T_adj",]
ggdft <- ggdft[ggdft$Region!="T_bord",]
...
pdf(paste0(workd,"/Results/Abundance_stackedbar_Type_T_noTLS.pdf"), width=7, height=5); bp; dev.off()
...
saveRDS(expr_Normal,'backup_dist_Normal_T_noTLS.rds')

#load previously saved distance matrix

expr_Normal<- readRDS('backup_dist_Normal_T_noTLS.rds')
...
saveRDS(expr_CP,'backup_dist_CP_T_noTLS.rds')

#load previously saved distance matrix

expr_CP<- readRDS('backup_dist_CP_T_noTLS.rds')
...
saveRDS(expr_PanIN,'backup_dist_PanIN_T_noTLS.rds')

#load previously saved distance matrix

expr_PanIN<- readRDS('backup_dist_PanIN_T_noTLS.rds')
...
saveRDS(expr_PDAC,'backup_dist_PDAC_T_noTLS.rds')

#load previously saved distance matrix

expr_PDAC<- readRDS('backup_dist_PDAC_T_noTLS.rds')
...
pdf(paste0(workd,"/Results/Distance_Relationships_T_noTLS.pdf"),width=10,height=8)
