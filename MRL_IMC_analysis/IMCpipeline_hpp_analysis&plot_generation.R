...
# From Melissa - I changed numclusters to 50 since that is what is currently on my HumanPanIN_merge
...
plot_clustering_heatmap_wrapper2 <- function(fcs, cell_clustering, nclusters=50,
                                             color_clusters='auto',
...
# output <- returnfcs(metaDataFile = paste0(work,"/Config/HumanPanIn_metadata.xlsx"),
#                     panelDataFile = paste0(work,"/Config/HumanPanIN_panel.xlsx"),
...
md <- read_excel("~/Config/HumanPanIn_metadata.xlsx")
output$meta_data <- md

## Set up levels

samplevels=c("1C_28500_1_n_1",
             "1C_28500_1_nad_1",
             "1C_28500_2_n_1",
             "1C_28500_2_nad_1",
             "1C_28500_3_n_1",
...
pdf('Diagnostics_cellcounts.pdf',width=20, height=20);ggp; dev.off()
...
output[(length(output)+1):(length(output)+3)] <- clusterfcs(fcs=output$fcs2, numclusters=50, scaleoption = T) 
...
## Load merge file
## Assign colors
clusterMergeFile = paste0(work,"/Config/HumanPanIN_merge.xlsx") #create dummy merger numbers prior to annotation
cluster_merging <- read_excel(clusterMergeFile)

clusterlevels=c(1:24)
...
plot_clustering_heatmap_wrapper(fcs=output$fcs2,
                                ...
                                fileName = 'Clusteringheatmap_all_50_fcs2_annotated.pdf'); dev.off()

# For merge
clusterMergeFile = paste0(work,"/Config/HumanPanIN_merge.xlsx") 
cluster_merging <- read_excel(clusterMergeFile)

clusterlevels=c("Macrophages",
                "CD4+ CD45RO+",
                "CD8+ GZMB+ KI67+ LAG3+",
                "CD8+",
                "CD8+ CD45RO+",
                "CD4+ CD45RO+ CD69+ TOX2+ PD1+",
                "CD4+ CD45RA+ CD20+",
                "GC B cells",
                "CD8+ GZMB+",
                "CD4+ CD45RO+ TOX2+ PD1+",
...
# For merge2
clusterMergeFile = paste0(work,"/Config/HumanPanIN_merge2.xlsx") 
cluster_merging <- read_excel(clusterMergeFile)

clusterlevels=c("B cells",
                "GC B cells",
                "Macrophages",
                "Dendritic cells",
                "CD4+ T cells",
...
# For merge3
clusterMergeFile = paste0(work,"/Config/HumanPanIN_merge3.xlsx") 
cluster_merging <- read_excel(clusterMergeFile)
...
# For merge4
clusterMergeFile = paste0(work,"/Config/HumanPanIN_merge4.xlsx") 
cluster_merging <- read_excel(clusterMergeFile)

clusterlevels=c("CD20+ CD21+ CD23+",
                "CD20+ CD45RA+",
...
plot_clustering_heatmap_wrapper2(fcs=output$fcs2,
                                ...
                                fileName = 'Clusteringheatmap_merge4_50_fcs2_annotated.pdf');dev.off()
...
dev.off()
pdf('Umaps_50_fcs2_merge4.pdf',width=20,height=20)
...
## Save output list
# saveRDS(output, file="backup_output_50_fcs2_merge4.rds")
# saveRDS(umapRes, file="backup_umap_50_fcs2.rds")
# umapRes<-readRDS('backup_umap_50_fcs2.rds')
...
PlotPerArea <- function(CELLTYPE, XAXIS, COMPARISON) {
  ...
  print (ggplot(ggdf,aes(x=get(XAXIS),y=density,fill=get(XAXIS)))+
           geom_boxplot(outlier.shape=NA, lwd=0.3, color="black")+
           geom_jitter(aes(size = 1), width=0.2, size=0.5)+
           ggtitle(paste(title))+
           ylab("cells/mm^2")+
           ...
           theme(axis.text.x = element_text(size=6, color="black", angle = 45, vjust = 1, hjust = 1),
                 ...
                 axis.title.y = element_text(size=6, angle = 90, color="black"),
                 strip.background = element_rect(fill=NA),
                 strip.text = element_text(size=6, color="black"),
                 plot.title = element_text(size = 6, face = "bold", vjust = 1, hjust = 0),
...
# Variables in PlotPerRatio function
#For each celltype, iterate through plotting
...
PlotRatioPerArea <- function(CELLTYPE, CELLTYPE2, XAXIS, COMPARISON) {
  ...
  # Find the number of cells per um2 of each region
  density <- (as.numeric(ggdf2[,"counts"])*1000000)/as.numeric(as.character(ggdf2[,"Area"]))
...
### Cell types from Merge4 to use to make the plots running the next function
...
# Find the number of cells per 100um2 of each region
density <- (as.numeric(ggdf[,"counts"])*1000000)/as.numeric(as.character(ggdf[,"Area"]))
...
# Set up the dataframe to plot only nonTLS samples

##Extract to get sampleIDs as a dataframe
subsetting_metadata <- as.data.frame(output$sample_ids)
##Extract nonTLS names from sampleID that don't have _t_, _panin_ or _cp_
# This removes all TLS samples and all panin, cp or pdac adjacent samples
tme_cells_nontls <- grep("tls", output$sample_ids,value=TRUE, invert = T)
tme_cells_nontls <- grep("ta", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("tb", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("_pa_", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("cpb", tme_cells_nontls,value=TRUE,invert=T)
# Need to remove these "_t_" samples from the analysis since this patient has cholangio and not PDAC
tme_cells_nontls <- grep("1M_28500_5_t_1_1", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("1M_28500_6_t_1_1", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("1D_28500_2_t_1_1", tme_cells_nontls,value=TRUE,invert=T)


##If the sampleID is present in the nonTLS names, then we write TRUE, otherwise FALSE
subsetting_metadata$include <- ifelse(subsetting_metadata$`output$sample_ids` %in% tme_cells_nontls, TRUE, FALSE)
##Subset everything based on this ^ true and false vector
tme_clusters <- output$cell_clustering1m[subsetting_metadata$include]
tme_sampleids <- output$sample_ids[subsetting_metadata$include]
...
# "CD8+ GZMB+", "CD68+ CD16+ CD11c+", "CD4+ CD45RO+", "CD8+ CD45RO+", "CD4+ CD45RO+ TOX2+ PD1+",
# "DCSIGN+", "CD4+ CD45RA+", "CD4+ CD45RA+ KI67+", "CD8+", "CD4+ FOXP3+ PD1-", "CD68+ CD16+ HLADR+",
# "CK+", "CK+ PDL1+", "CD8+ GZMB+ KI67+ LAG3+", "PDPN+", "COL+ SMA+ VIM+", "Unassigned", "CD57+", "CD45+", "CD57+ TOX2+"

pdf('CD4+ CD45RO+ CCR7+ HLADR+_boxplots_perarea_byregion_nontls_pdacptsonly.pdf',width=1.25,height=2)

PlotPerArea("CD4+ CD45RO+ CCR7+ HLADR+","Type", type_comp)

dev.off()

# Look at ratio of densities

pdf('cd4_cd8_ratio_boxplots_perarea_byregion_nontls_pdacptsonly.pdf',width=2.5,height=4)

PlotRatioPerArea("CD4+ T cells", "CD8+ T cells","Type", type_comp)

dev.off()


###### For nonTLS samples of the tumor adjacent/border samples specifically

##Extract to get sampleIDs as a dataframe
subsetting_metadata <- as.data.frame(output$sample_ids)
##Extract nonTLS names from sampleID that don't have _t_ 
# (only want to plot intra-tumor PDAC samples)
tme_cells_nontls <- grep("tls", output$sample_ids,value=TRUE, invert = T)
tme_cells_nontls <- grep("_n_", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("_nad_", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("_panin_", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("_pa_", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("_cp_", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("_cpb_", tme_cells_nontls,value=TRUE,invert=T)
# Need to remove these samples from the analysis since this patient has cholangio and not PDAC
tme_cells_nontls <- grep("1M_28500_5_t_1_1", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("1M_28500_6_t_1_1", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("1D_28500_2_t_1_1", tme_cells_nontls,value=TRUE,invert=T)
...
pdf('CD4+ FOXP3+ PD1-_boxplots_perarea_bycondition_nontls_tumorvstumoradj.pdf',width=2.5,height=4)

PlotPerArea("CD4+ FOXP3+ PD1-","Condition", cond_comp2)
...
pdf('cd8_tregs_ratio_boxplots_perarea_byregion_nontls.pdf',width=2.5,height=4)

PlotRatioPerArea("CD8+ T cells", "Tregs","Type", type_comp)
...
# To look at percentage of different cell types in TA vs T not in TLSs
##Extract to get sampleIDs as a dataframe
subsetting_metadata <- as.data.frame(output$sample_ids)
##Extract nonTLS names from sampleID that are not intratumoral or tumor adjacent/border
tme_cells_nontls <- grep("tls", output$sample_ids,value=TRUE, invert = T)
tme_cells_nontls <- grep("_n_", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("_nad_", tme_cells_nontls,value=TRUE,invert=T)
tme_cells_nontls <- grep("_panin_", tme_cells_nontls,value=TRUE,invert=T)
...
# Subset for only CD3+ cells to get a percentage of immune cells for this stacked bp
counts_table_cd3 <- counts_table[rownames(counts_table) %in% c("CD4+ CD45RO+ CCR7+ HLADR+",
                                        "CD8+ GZMB+",
                                        "CD4+ CD45RO+",
                                        "CD8+ CD45RO+",
                                        "CD4+ CD45RO+ TOX2+ PD1+",
                                        ...
                                        "CD8+",
                                        "CD4+ FOXP3+ PD1-",
                                        # Didn't have any cells in this data set from this cluster
                                        # "CD8+ GZMB+ KI67+ LAG3+",
                                        "CD45+"),]

counts <- as.data.frame.matrix(counts_table_cd3)


# Plot for the last props made above based on which samples you want to include

  # Colors for this plot
  
  clusternames<-sort(unique(rownames(counts)))
  
  colorassigned<-sample(kovesi.rainbow_bgyrm_35_85_c69(length(unique(rownames(counts)))))
  ...
  # Calculate the actual percentages that will be plotted in ggplot stacked bp
  
  dt <- ggdf %>%
    dplyr::group_by(Condition, cluster)%>%
    dplyr::summarize(sum = sum(density))%>%
    # dplyr::tally()%>%
    dplyr::mutate(percent=(sum/sum(sum))*100)
  
  write.csv(dt, "stackedbp_t_ta_percentofCD3+cells_pdacpts.csv")

  # Calculate proportion of CD3+ cells per area
  
  # Used this to get a proportion of the density, but not actually necessary because ggplot just does it for you for positon = "fill"
  ggdf$proportionCD3 <- rep("unknown", nrow(ggdf))

  ggdf[ggdf$Condition == "TA",]$proportionCD3 <- ggdf[ggdf$Condition == "TA",]$density / sum(ggdf[ggdf$Condition == "TA",]$density)
  ggdf[ggdf$Condition == "T",]$proportionCD3 <- ggdf[ggdf$Condition == "T",]$density / sum(ggdf[ggdf$Condition == "T",]$density)

  ggdf$proportionCD3 <- as.numeric(ggdf$proportionCD3)


  # Calculate stats for this; have to run each against each other separately
  sink("stackedbp_t_ta_percentofCD3+cells_pdacpts_wilcoxstats.csv")
  for(i in unique(ggdf$cluster)){
    print(i)
  print(wilcox.test(as.numeric(ggdf[ggdf$Condition=="T" & ggdf$cluster==i,]$proportionCD3), 
                    as.numeric(ggdf[ggdf$Condition=="TA" & ggdf$cluster==i,]$proportionCD3)))
  }
  sink(file=NULL)
  ...
  bp <- ggplot(dt, aes(x = Condition, y = sum, fill=cluster, order=cluster)) +
    geom_bar(stat = "identity", position="stack", width=0.8) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 0, vjust= 0.5, hjust = 0.25, color="black", size=16),
  ...
  pdf('stackedbp_t_ta_proportionofcd3+_pdacpts.pdf')  
  bp
dev.off()



# To look at percentage of different cell types in nonTLS colored by the patient
# that the sample originated
# Using merge3
##Extract to get sampleIDs as a dataframe
subsetting_metadata <- as.data.frame(output$sample_ids)
##Extract nonTLS names from sampleID that don't have _t_, _panin_ or _cp_
...
#For a plot of the cluster breakdown
# Colors for this plot
...
bp <- ggplot(ggdf, aes(x = Type, y = counts, fill=Patient, order = Patient)) +
  geom_bar(stat = "identity", position="stack", width=0.8) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 0, vjust= 0.5, hjust = 0.25, color="black", size=16),
        axis.text.y = element_text(color="black", size = 16),
  ...
  # scale_y_continuous(labels = scales::percent, expand = c(0,0))+
  guides(fill=guide_legend(ncol=1)) 
pdf('stackedbp_merge3_bytype_stacked_pdacpts_coloredbypts.pdf')  
bp
dev.off()



# To look at percentage of different cell types in TLSs according to each patient
# Using merge3
##Extract TLS names from sampleID that don't have _t_
tme_cells_tls <- grep("tls", output$sample_ids,value=TRUE)
tme_cells_tls <- grep("_t_", tme_cells_tls,value=TRUE,invert=T)
...
bp <- ggplot(ggdf, aes(x = Type, y = counts, fill=cluster, order=cluster)) +
  ...
  scale_y_continuous(labels = scales::percent, expand = c(0,0))+
  guides(fill=guide_legend(ncol=1)) 
pdf('stackedbp_merge3_bytype_filled_tls_pdacpts.pdf')  
bp
dev.off()
...
# Create a table summarising all of the counts and proportions from all of the data
counts_table <- table(output$cell_clustering1m, output$sample_ids)
props_table <- t(t(counts_table) / colSums(counts_table)) * 100

samp <- table(output$cluster_by, output$sample_ids)


counts <- as.data.frame.matrix(counts_table)
props <- as.data.frame.matrix(props_table)

write.csv(counts,'Results_counts_50_fcs2.csv')
write.csv(props,'Results_props_50_fcs2.csv')
...
#% CELLS TLS
ggp2<-ggplot(ggdf,aes(x=Condition,y=proportion,fill=TLS_status))+
  geom_boxplot(outlier.shape=NA, lwd=0.25, color="black")+
  geom_jitter(aes(shape=Patient), width=0.2, size=1)+
...
pdf('Abundance_box_tls_50_fcs2_ann.pdf',width=16,height=16)
ggp2
